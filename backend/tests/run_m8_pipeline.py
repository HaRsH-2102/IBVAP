"""
IBVAP — Milestone 8 Integration Validation
==========================================
Runs the complete M1->M8 pipeline on a video file.
Includes YOLO (M3), ByteTrack (M4), Spatial (M5), Event Interpretation (M6), Behavioral Intelligence (M7), and Night-Time Intelligence (M8).
"""
...
def run_m8_test(video_path: str, inference_size: int, conf_thresh: float, no_show: bool = False, simulate_night: bool = False, model_name: str = "yolov8n.pt") -> None:
    print(f"\n{'='*60}")
    print(f"Starting M8 Full Pipeline Validation:")
    print(f"Video: {video_path}")
    print(f"Model: {model_name}")
    print(f"Simulate Night: {simulate_night}")
    print(f"{'='*60}")
    ...
    m8_config.default_config.movement_duration_threshold_seconds = 1.0 # Fast for demo
    ...
    window_name = "IBVAP M8 - Night-Time Intelligence"
    if not no_show:
    ...
    try:
        while pipeline.is_running:
            ...
            if frame:
                ...
                # 4b. Night-Time Intelligence (M8)
                m8_start = time.perf_counter()
                
                # Darken frame if simulating night
                if simulate_night:
                    # Reduce brightness by 80% to force M8 scene analyzer into NIGHT mode
                    m8_frame = (frame.data * 0.2).astype(np.uint8)
                else:
                    m8_frame = frame.data
                    
                scene_state, night_events = m8_engine.process(camera_id, m8_frame, tracks, behavioral_events, current_time)
                m8_time = time.perf_counter() - m8_start
                total_m8_time += m8_time
                ...
                if not no_show:
                    ...
                    for t in tracks:
                        ...
                        # Override color if Night Movement
                        if state_key in m8_engine.movement_detector.track_states:
                            m8_state = m8_engine.movement_detector.track_states[state_key]
                            if m8_state.status == "EVENT_ACTIVE":
                                color = (0, 255, 255) # Yellow for Night Movement
                                behavior_txt = "NIGHT_MOVEMENT"
                    ...
                    avg_m8 = (total_m8_time / frames_processed) * 1000
                    stats_text = f"Src: {stats['source_fps']:.1f} FPS | Scene: {scene_state.name} | M8 Avg: {avg_m8:.2f}ms"
                    cv2.putText(vis_img, stats_text, (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    ...
    print("\n--- Final M8 Night-Time Engine Performance Report ---")
    avg_m7 = (total_m7_time / max(1, frames_processed)) * 1000
    avg_m8 = (total_m8_time / max(1, frames_processed)) * 1000
    print(f"Frames processed: {frames_processed}")
    print(f"M7 Avg latency per frame: {avg_m7:.3f} ms")
    print(f"M8 Avg latency per frame: {avg_m8:.3f} ms")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Milestone 8 Integration Validation")
    ...
    parser.add_argument("--simulate-night", action="store_true", help="Artificially darken the scene for M8 to trigger night logic")
    parser.add_argument("--model", type=str, default="yolov8n.pt", help="Model to use")
    
    args = parser.parse_args()
    run_m8_test(args.video, args.size, args.conf, args.no_show, args.simulate_night, args.model)
